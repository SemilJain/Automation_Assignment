...
module "private_ec2" {
  ...
  user_data = <<EOF
    ...
    RDS_MYSQL_ENDPOINT="${element(split(":", aws_db_instance.semil_RDS.endpoint), 0)}";
    RDS_MYSQL_USER="SemilJain";
    RDS_MYSQL_PASS="Qwerty12";
    RDS_MYSQL_BASE="semil-db";
    mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE -e 'quit';
    if [[ $? -eq 0 ]]; then
      echo "MySQL connection: OK" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='CREATE TABLE semil1( id INT)';
      echo "MySQL table create" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='INSERT into semil1 (id) VALUES (1)';
      echo "MySQL insert" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='SELECT * from semil1' >> conn;
...
module "semil_RDS" {
  ...
  subnet_group_name = "semil_subnet_group"
